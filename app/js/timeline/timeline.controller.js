...
class TimelineController {
  ...
  getVideoContainerClass(video) {
    if (video.direction == 'left') { return 'video-list--positioner--left'; }
    if (video.direction == 'right') { return 'video-list--positioner--right'; }
  }

  getVideoThumbnailClass(video) {
    const scale = video.snippet.thumbnails.scale;
    if (scale == 1) { return 'video--sm'; }
    if (scale == 2) { return 'video--md'; }
    if (scale == 3) { return 'video--lg'; }
    if (scale == 4) { return 'video--xl'; }
  }
